...
5. Identify useful, recent developer-focused updates from those sources. Keep summaries short and practical, and attribute each update to its source.
6. Use the `edit` tool to update `site/content/github-info.md` while preserving its existing format and unrelated content.
7. Review the resulting change for accuracy, concise wording, source links, and valid Markdown.
8. Use the `create-pull-request` safe output exactly once to open a pull request containing the change for Mona to review. Do not push directly to `main`.
